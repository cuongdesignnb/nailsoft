...
const concurrencyLevels = (process.env.LOAD_CONCURRENCY ?? "10,25").split(",").map(Number);
const email = process.env.LOAD_EMAIL ?? "owner@example.test";
const password = process.env.LOAD_PASSWORD ?? "DemoPass123!";
if (!email.endsWith("@example.test")) throw new Error("Load smoke refuses non-test credentials");

const loginBody = (deviceId) => ({ tenantSlug: "nailsoft-demo", email, password, deviceId, deviceName: "Load Smoke", platform: "android" });
async function request(path, init = {}) {
  ...
  } catch { return { latency: performance.now() - start, status: 0 }; }
}
async function auth(deviceId) {
  const response = await fetch(`${baseUrl}/v1/auth/login`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(loginBody(deviceId)) });
...
const scenarios = [
  ...
  { name: "login", run: (_, worker) => request("/v1/auth/login", { method: "POST", body: JSON.stringify(loginBody(`load-login-${worker}`)) }) },
...
for (const concurrency of concurrencyLevels) {
  for (const scenario of scenarios) {
    const states = await Promise.all(Array.from({ length: concurrency }, (_, worker) => scenario.setup?.(worker) ?? {}));
    await execute(scenario, states, concurrency, warmupMs, false);
    const report = await execute(scenario, states, concurrency, durationMs, true);
    process.stdout.write(`${JSON.stringify(report)}\n`);
  }
}

async function execute(scenario, states, concurrency, windowMs, collect) {
  const samples = []; let errors = 0; let timeouts = 0; const end = Date.now() + windowMs;
  await Promise.all(states.map(async (state, worker) => {
    while (Date.now() < end) {
      const result = await scenario.run(state, worker);
      if (collect) samples.push(result.latency);
      const expected = scenario.expected?.includes(result.status) || (result.status >= 200 && result.status < 400);
  ...
  return { scenario: scenario.name, concurrency, durationSeconds: windowMs / 1000, requestCount: samples.length,
    throughput: Number((samples.length / (windowMs / 1000)).toFixed(2)), p50: percentile(.50), p95: percentile(.95), p99: percentile(.99),
    errorRate: samples.length ? errors / samples.length : 0, timeouts, cpuCount: cpus().length,
    processMemoryMb: Math.round(process.memoryUsage().rss / 1024 / 1024), systemMemoryUsedMb: Math.round((totalmem()-freemem())/1024/1024) };
}
