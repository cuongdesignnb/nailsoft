...
`DRAFT -> PENDING_APPROVAL|CANCELLED`; `PENDING_APPROVAL -> APPROVED|CANCELLED`; `APPROVED -> SCHEDULED|CANCELLED`; `SCHEDULED -> RUNNING|PAUSED|CANCELLED|FAILED`; `RUNNING -> PAUSED|COMPLETED|CANCELLED|FAILED`; `PAUSED -> RUNNING|CANCELLED`.
