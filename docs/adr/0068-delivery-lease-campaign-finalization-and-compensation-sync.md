...
- `PAUSED` campaigns cannot be claimed. Only a `RUNNING` campaign may transition to `PAUSED`; a `SCHEDULED` campaign may start or be cancelled but cannot be paused. `CANCELLED` campaigns terminalize unsent messages and audience evidence. A replay-safe finalizer derives `COMPLETED` and immutable terminal counters only while the campaign is `RUNNING` and every current-generation recipient is terminal.
