...
export default function StaffMfa() {
  ...
  const pending = pendingStaffMfa();
  const [code, setCode] = useState("");
  const [recovery, setRecovery] = useState(false);
  const [enrollment, setEnrollment] = useState<{ otpauthUri?: string; secret?: string }>();
  const [recoveryCodes, setRecoveryCodes] = useState<string[]>([]);
  const [error, setError] = useState("");
  if (!pending) return <SafeAreaView style={styles.screen}><Text accessibilityRole="alert">MFA challenge has expired. Sign in again.</Text><NativeButton label={staffText(undefined, "signIn")} onPress={() => router.replace("/")} /></SafeAreaView>;
  const submit = async () => {
    try {
    ...
    } catch (reason: unknown) { setError(reason instanceof Error ? reason.message : "MFA failed safely."); }
  };
