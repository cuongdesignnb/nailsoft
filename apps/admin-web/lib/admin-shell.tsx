  ...
  return <div className={`ns-app-frame ${compact ? "ns-app-frame--compact" : ""}`}>
    ...
    <div className="ns-app-body">
      <header className="ns-admin-header">
        ...
        <div className="ns-breadcrumb"><span>{platformRoute ? "PLATFORM ADMIN" : context.workspace.tenantName}</span><span aria-hidden="true">/</span><strong>{page ? translate(locale, page.label) : translate(locale, "dashboard")}</strong></div>
        <div className="ns-header-actions">
          {!platformRoute && <label className="ns-branch-picker"><span className="sr-only">{translate(locale, "branch")}</span><Icon name="store" /><select value={branchId ?? ""} onChange={(event) => selectBranch(event.target.value || undefined)}>{authorizedBranches.length === 0 || authorizedBranches.length > 1 ? <option value="" aria-label={translate(locale, "workspace")}>Workspace</option> : null}{authorizedBranches.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select></label>}
          <StatusBadge tone={context.workspace.accessMode === "FULL" || context.workspace.accessMode === "GRACE" ? "success" : "warning"}>{platformRoute ? (context.supportAccess ? `SUPPORT SESSION · ${context.workspace.tenantName}` : "PLATFORM ADMIN") : selectedBranch?.name ?? context.workspace.accessMode}</StatusBadge>
