...
function errorText(error: unknown) {
  ...
  const knownMessages: Record<string, string> = {
    "Internal server error": "Máy chủ chưa trả về được báo cáo doanh thu.",
    Unauthorized: "Phiên đăng nhập không còn hiệu lực.",
    Forbidden: "Bạn không có quyền xem báo cáo doanh thu.",
  };
  return knownMessages[message] ?? (message.toLowerCase().includes("currency") ? "Cần chọn phạm vi tiền tệ phù hợp." : "Không thể tải dữ liệu doanh thu.");
}
...
export default function NetSalesPage() {
  ...
  return <main className={styles.page}>
    ...
    {error ? <div className={styles.alertError} role="alert"><UiIcon name="alert" /> {error}<button onClick={() => setReloadKey((value) => value + 1)}>Thử lại</button></div> : null}
    ...
    {error ? <section className={styles.errorDataCard} role="status"><UiIcon name="alert" /><strong>Không thể hiển thị số liệu doanh thu</strong><span>API báo cáo chưa trả về dữ liệu hợp lệ. Các ô tiền và KPI được ẩn để không nhầm lỗi tải với số liệu hợp lệ.</span></section> : <>
    ...
    {loading && !overview ? <div className={styles.loadingState}><div /><div /><div /></div> : error ? null : overview && hasData ? <>
