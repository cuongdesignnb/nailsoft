...
function AudienceBar({ audience }: { audience: any }) {
  ...
  return <div className={styles.audienceCell}><div className={styles.audienceBar} aria-label={`Audience ${count(total)}`}><span style={{ flex: Math.max(0, sent) }} /><span style={{ flex: Math.max(0, suppressed) }} /><span style={{ flex: Math.max(0, failed) }} /><span style={{ flex: Math.max(0, pending) }} /></div><small>{count(total)} snapshot · {count(sent)} gửi</small></div>;
}
...
function CampaignCreateDrawer({ open, onClose, onSaved, branches, segments, templates, context, mutation, allowTenantWide = Boolean(context?.tenantWideAllowed) }: { open: boolean; onClose: () => void; onSaved: () => void; branches: any[]; segments: any[]; templates: any[]; context: any; mutation: ReturnType<typeof useMarketingMutation>; allowTenantWide?: boolean }) {
  ...
  useEffect(() => {
    ...
    setCampaignType((current) => current || String(context?.campaignTypes?.[0] ?? "PROMOTION"));
    setRiskLevel((current) => current || String(context?.riskLevels?.[0] ?? "STANDARD"));
...
function CampaignInspector({ detail, preview, onAction, canApprove, canSchedule, canCancel, canCreate, canReport, mutation }: { detail: any; preview: any; onAction: (action: string, scheduledAt?: string) => void; canApprove: boolean; canSchedule: boolean; canCancel: boolean; canCreate: boolean; canReport: boolean; mutation: ReturnType<typeof useMarketingMutation> }) {
  ...
  const actions = status === "DRAFT" ? ["preview", "submit", "cancel"] : status === "PENDING_APPROVAL" ? ["preview", "approve", "cancel"] : status === "APPROVED" ? ["schedule", "cancel"] : status === "SCHEDULED" ? ["view_schedule", "cancel"] : status === "RUNNING" ? ["pause"] : status === "PAUSED" ? ["resume", "cancel"] : status === "COMPLETED" ? [canReport ? "report" : "history"] : status === "FAILED" ? ["failure"] : ["history"];
