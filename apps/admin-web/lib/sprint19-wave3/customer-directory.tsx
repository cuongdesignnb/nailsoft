...
function StatePanel({ state, error, retry, label }: { state: DirectoryState; error: string; retry: () => void; label: string }) {
  ...
  if (state === "offline") return <div className="s19-state" role="alert"><strong>Đang ngoại tuyến</strong><span>Dữ liệu khách hàng chưa thể tải khi không có kết nối.</span><button className="s19-button s19-button-secondary" type="button" onClick={retry}>Thử lại</button></div>;
  ...
  if (state === "empty") return <div className="s19-state" role="status"><strong>Chưa tìm thấy khách hàng</strong><span>Thử tìm theo tên, số điện thoại hoặc email khác.</span><button className="s19-button s19-button-secondary" type="button" onClick={retry}>Tải lại</button></div>;
  return null;
}
  ...
  return (
    <main className="s19-customer-page">
      <header className="s19-page-heading">
        <div><p className="s19-eyebrow">KHÁCH HÀNG · CUSTOMER 360</p><h1>Danh sách khách hàng</h1><p>Tra cứu khách hàng trong phạm vi được cấp quyền và mở hồ sơ 360 đầy đủ.</p></div>
        <div className="s19-page-actions"><a className="s19-button s19-button-primary" href="/admin/customers/new">Tạo khách hàng</a></div>
      </header>
      <section className="s19-card s19-customer-directory-card">
        ...
        {directory.errorCode === "INVALID_CUSTOMER_CURSOR" ? <div className="s19-notice s19-notice-error" role="alert">Trang danh sách đã hết hiệu lực. Hãy tìm kiếm lại để bắt đầu phân trang mới.</div> : null}
        <StatePanel state={directory.state} error={directory.error} retry={() => void directory.load()} label="danh sách khách hàng" />
        {directory.state === "ready" ? <>
          ...
          <div className="s19-customer-pagination"><span>Hiển thị {directory.rows.length} khách hàng</span>{directory.hasMore ? <button className="s19-button s19-button-secondary" type="button" onClick={() => void directory.loadMore()} disabled={directory.loadingMore}>{directory.loadingMore ? "Đang tải…" : "Tải thêm"}</button> : <span>Đã hiển thị hết kết quả</span>}</div>
        </> : null}
      </section>
    </main>
  );
