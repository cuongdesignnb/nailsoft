...
export function GiftCardProducts() {
  ...
  return <BenefitShell title="Gift card products" eyebrow="STORED VALUE · PRODUCTS" backHref="/admin/gift-cards"><div className="s19-card-heading"><div><p className="s19-helper">Cấu hình sản phẩm bán; việc phát hành vẫn thực hiện trong luồng cấp vốn POS.</p></div><button className="s19-button s19-button-primary" onClick={() => setShowForm((value) => !value)}>{showForm ? "Đóng biểu mẫu" : "Tạo sản phẩm"}</button></div>{showForm && <form className="s19-card s19-benefit-form" onSubmit={(event) => void submit(event)}><div className="s19-form-grid"><Field label="Mã sản phẩm"><input required value={String(form.productCode)} onChange={(event) => set("productCode", event.target.value)} /></Field><Field label="Tên hiển thị"><input required value={String(form.name)} onChange={(event) => set("name", event.target.value)} /></Field><Field label="Cách nhập mệnh giá"><select value={String(form.amountMode)} onChange={(event) => set("amountMode", event.target.value)}><option value="OPEN">Số tiền mở</option><option value="FIXED">Mệnh giá cố định</option></select></Field><Field label="Hình thức thẻ"><select value={String(form.cardForm)} onChange={(event) => set("cardForm", event.target.value)}><option value="BOTH">Vật lý hoặc điện tử</option><option value="PHYSICAL">Vật lý</option><option value="DIGITAL">Điện tử</option></select></Field><Field label="Tiền tệ"><input required maxLength={3} value={String(form.currency)} onChange={(event) => set("currency", event.target.value.toUpperCase())} /></Field><Field label="Mệnh giá tối thiểu (đơn vị nhỏ nhất)"><input required value={String(form.minimumAmountMinor)} onChange={(event) => set("minimumAmountMinor", event.target.value)} /></Field><Field label="Mệnh giá tối đa (đơn vị nhỏ nhất)"><input required value={String(form.maximumAmountMinor)} onChange={(event) => set("maximumAmountMinor", event.target.value)} /></Field><Field label="Số dư tối đa (đơn vị nhỏ nhất)"><input required value={String(form.maximumBalanceMinor)} onChange={(event) => set("maximumBalanceMinor", event.target.value)} /></Field><Field label="Mệnh giá cố định (cách nhau bằng dấu phẩy)" wide><input value={String(form.fixedDenominationsMinor)} onChange={(event) => set("fixedDenominationsMinor", event.target.value)} /></Field><Field label="Chính sách gán khách"><select value={String(form.assignmentPolicy)} onChange={(event) => set("assignmentPolicy", event.target.value)}><option value="BEARER_OR_CUSTOMER">Bearer hoặc khách hàng</option><option value="CUSTOMER_REQUIRED">Bắt buộc gán khách</option><option value="BEARER">Bearer</option></select></Field><Field label="Cho phép nạp thêm"><input type="checkbox" checked={Boolean(form.reloadable)} onChange={(event) => set("reloadable", event.target.checked)} /></Field><Field label="Bắt buộc PIN"><input type="checkbox" checked={Boolean(form.pinRequired)} onChange={(event) => set("pinRequired", event.target.checked)} /></Field></div><div className="s19-inline-actions"><button className="s19-button s19-button-primary" disabled={mutation.state === "submitting"}>{mutation.state === "submitting" ? "Đang lưu…" : "Lưu bản nháp"}</button>{mutation.message && <span className={mutation.state === "error" ? "s19-notice s19-notice-danger" : "s19-notice s19-notice-success"}>{mutation.code ? `${mutation.code}: ` : ""}{mutation.message}</span>}</div></form>}<BenefitStatePanel resource={resource} label="sản phẩm Gift Card" />{resource.state === "ready" && <div className="s19-benefit-table-wrap"><table className="s19-benefit-table"><caption className="s19-sr-only">Sản phẩm Gift Card</caption><thead><tr><th>Sản phẩm</th><th>Mệnh giá</th><th>Hình thức</th><th>Chính sách</th><th>Trạng thái</th><th>Thao tác</th></tr></thead><tbody>{products.map((product) => <tr key={product.id}><td data-label="Sản phẩm"><strong>{product.productCode}</strong><small>{localized(product.name)}</small></td><td data-label="Mệnh giá">{formatMoney(product.minimumAmountMinor, product.currency)} – {formatMoney(product.maximumAmountMinor, product.currency)}</td><td data-label="Hình thức">{statusLabel(product.cardForm)}</td><td data-label="Chính sách">{product.reloadable ? "Có thể nạp thêm" : "Nạp một lần"}<small>{statusLabel(product.assignmentPolicy)}</small></td><td data-label="Trạng thái"><span className="s19-status s19-status-info">{statusLabel(product.status)}</span><small>v{product.version}</small></td><td data-label="Thao tác">{product.status === "DRAFT" && <button className="s19-button s19-button-small" onClick={() => void activate(product)}>Kích hoạt</button>}</td></tr>)}</tbody></table></div>}</BenefitShell>;
}

export function GiftCardIssuance() {
  const products = useBenefitResource("/v1/gift-card-products");
  return <BenefitShell title="Gift card issuance" eyebrow="STORED VALUE · POS HANDOFF" backHref="/admin/gift-cards"><section className="s19-card"><div className="s19-card-heading"><div><p className="s19-eyebrow">PHÁT HÀNH AN TOÀN</p><h2>Bắt đầu bán Gift Card tại POS</h2></div><span className="s19-status s19-status-info">Yêu cầu đã thu tiền</span></div><p className="s19-helper">Gift Card chỉ được tạo qua luồng cấp vốn POS hiện có sau khi thanh toán được thu hợp lệ. Trang này không tạo thẻ trực tiếp và không xử lý PIN hoặc số thẻ đầy đủ.</p><div className="s19-inline-actions"><a className="s19-button s19-button-primary" href="/admin/pos/new">Mở POS để bán Gift Card</a><a className="s19-button s19-button-secondary" href="/admin/gift-cards">Xem Gift Card đã phát hành</a></div></section><section className="s19-card"><h2>Sản phẩm khả dụng</h2><BenefitStatePanel resource={products} label="sản phẩm Gift Card" />{products.state === "ready" && <div className="s19-benefit-stack">{rows(products.data).filter((product) => product.status === "ACTIVE").map((product) => <div className="s19-benefit-item" key={product.id}><div><strong>{product.productCode} · {localized(product.name)}</strong><span>{product.currency} · {statusLabel(product.cardForm)} · {product.reloadable ? "Có thể nạp thêm" : "Nạp một lần"}</span></div><b>{formatMoney(product.minimumAmountMinor, product.currency)} – {formatMoney(product.maximumAmountMinor, product.currency)}</b></div>)}</div>}</section></BenefitShell>;
}
