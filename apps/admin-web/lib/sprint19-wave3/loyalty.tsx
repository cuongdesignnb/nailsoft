...
import { BenefitShell, BenefitStatePanel, CustomerBenefitHeader, LedgerTable, formatDate, formatInteger, formatMoney, localized, rows, statusLabel, useBenefitMutation, useBenefitResource } from "./benefit-shared";
...
export function CustomerLoyalty({ customerId }: { customerId: string }) {
  const account = useBenefitResource(`/v1/customers/${encodeURIComponent(customerId)}/loyalty`);
  const ledger = useBenefitResource(`/v1/customers/${encodeURIComponent(customerId)}/loyalty/ledger`);
  return <BenefitShell title="Customer loyalty ledger" eyebrow="CUSTOMER BENEFITS" backHref={`/admin/benefits/customers/${customerId}`}><CustomerBenefitHeader customerId={customerId} backHref={`/admin/benefits/customers/${customerId}`} /><div className="s19-benefit-grid"><section className="s19-benefit-panel"><div className="s19-benefit-panel-heading"><div><p className="s19-eyebrow">BALANCE</p><h2>Points account</h2></div></div><BenefitStatePanel resource={account} label="points account" />{account.state === "ready" ? <dl className="s19-benefit-metrics"><div><dt>Available</dt><dd>{formatInteger(account.data?.availablePoints)}</dd></div><div><dt>Pending</dt><dd>{formatInteger(account.data?.pendingPoints)}</dd></div><div><dt>Reserved</dt><dd>{formatInteger(account.data?.reservedPoints)}</dd></div><div><dt>Lifetime earned</dt><dd>{formatInteger(account.data?.lifetimeEarnedPoints)}</dd></div></dl> : null}</section><section className="s19-benefit-panel"><div className="s19-benefit-panel-heading"><div><p className="s19-eyebrow">APPEND-ONLY</p><h2>Ledger history</h2></div></div><BenefitStatePanel resource={ledger} label="loyalty ledger" />{ledger.state === "ready" ? <LedgerTable entries={rows(ledger.data)} emptyLabel="loyalty ledger entries" /> : null}</section></div></BenefitShell>;
}
