...
export default function Sprint19Wave4Screen({ pathname }: { pathname: string }) {
  ...
  if (pathname === "/admin/staff/list" || pathname === "/admin/time-clock" || pathname === "/admin/payroll/runs" || /^\/admin\/payroll\/runs\/[^/]+$/.test(pathname)) return <WorkforceHub pathname={pathname} />;
  if (pathname === "/admin/staff/new") return <StaffWorkspace pathname={pathname} />;
  ...
  if (pathname.startsWith("/admin/time-clock") || pathname === "/admin/timesheets" || pathname === "/admin/timesheet-periods" || /^\/admin\/timesheets\/[^/]+$/.test(pathname)) return <AttendanceWorkspace pathname={pathname} />;
