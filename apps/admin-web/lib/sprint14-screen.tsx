...
export default function Sprint14Screen(){const pathname=usePathname();const view=views[pathname]??views["/admin/accounting"]!;const [state,setState]=useState<State>("loading"),[rows,setRows]=useState<any[]>([]),[error,setError]=useState("");const load=useCallback(async()=>{setState("loading");setError("");try{const value=await read(view.endpoint);const list=Array.isArray(value)?value:value?[value]:[];setRows(list);setState(list.length?"ready":"empty");}catch(e:any){setError(e.message);setState(e.forbidden?"forbidden":"error");}},[view.endpoint]);useEffect(()=>void load(),[load]);const cols=useMemo(()=>Array.from(new Set(rows.flatMap(r=>Object.keys(r)))).filter(k=>!k.toLowerCase().includes("json")).slice(0,8),[rows]);
  return <main className="shell ops-shell"><nav className="topbar">{Object.entries(views).map(([href,v])=><a key={href} href={href}>{v.title}</a>)}</nav><section className="card"><p className="eyebrow">SPRINT 14 · GENERAL LEDGER</p><div className="title-row"><div><h1>{view.title}</h1><p>{view.hint}</p></div><button onClick={()=>void load()}>Refresh</button></div>{state==="loading"&&<p aria-busy="true">Loading authoritative accounting data…</p>}{state==="forbidden"&&<p role="alert">Permission denied for this accounting scope.</p>}{state==="error"&&<div role="alert"><p>{error}</p><button onClick={()=>void load()}>Retry</button></div>}{state==="empty"&&<p>No records yet. This is a valid empty state.</p>}{state==="ready"&&<div className="table-wrap"><table><thead><tr>{cols.map(c=><th key={c}>{c}</th>)}{view.actions&&<th>Commands</th>}</tr></thead><tbody>{rows.map((row,i)=><tr key={row.id??i}>{cols.map(c=><td key={c}>{typeof row[c]==="object"?JSON.stringify(row[c]):String(row[c]??"—")}</td>)}{view.actions&&<td>{view.actions.map(action=><button key={action} onClick={()=>setError(`${action} requires the selected record and explicit confirmation.`)}>{action}</button>)}</td>}</tr>)}</tbody></table></div>}</section><aside className="card"><h2>Accounting safeguards</h2><p>Postings are immutable after posting, period closure is dual-control, and PostgreSQL remains the source of truth.</p></aside></main>;
}
