...
export default function Sprint15Screen() {
  ...
  return <main className="shell ops-shell"><nav className="topbar">{nav.map(([href, item]) => <a key={href} href={href}>{item.title}</a>)}</nav><section className="card"><p className="eyebrow">SPRINT 15 · PROCUREMENT & AP</p><div className="title-row"><div><h1>{view.title}</h1><p>{view.hint}</p></div><button onClick={() => void load()}>Refresh</button></div>{notice && <p role="status">{notice}</p>}{state === "loading" && <p aria-busy="true">Loading authoritative procurement data…</p>}{state === "forbidden" && <p role="alert">Permission denied. Your role or branch scope does not allow this view.</p>}{state === "error" && <div role="alert"><p>{error}</p><button onClick={() => void load()}>Retry</button></div>}{state === "empty" && <div><p>No records yet.</p><button onClick={() => void load()}>Retry</button></div>}{state === "ready" && <div className="table-wrap"><table><thead><tr>{columns.map((column) => <th key={column}>{column}</th>)}<th>Actions</th></tr></thead><tbody>{rows.map((row, index) => <tr key={row.id ?? index}>{columns.map((column) => <td key={column}>{display(row[column])}</td>)}<td>{row.status === "SUBMITTED" && view.title === "Purchase requests" && <button onClick={() => void action(row, "approve")}>Approve</button>}{row.status === "PENDING_APPROVAL" && view.title === "Vendor payments" && <button onClick={() => void action(row, "approve")}>Approve</button>}{row.status === "APPROVED" && view.title === "Vendor payments" && <button onClick={() => void action(row, "process")}>Queue processing</button>}</td></tr>)}</tbody></table></div>}</section>{view.create && <section className="card"><h2>{view.create === "vendor" ? "Add vendor" : "New purchase request"}</h2><div className="form-grid">{fields.map(([name, label]) => <label key={name}>{label}<input value={form[name] ?? ""} onChange={(event) => setForm({ ...form, [name]: event.target.value })} required={name === "code" || name === "displayName" || name === "description"} /></label>)}</div><button onClick={() => void create()}>Save</button></section>}<aside className="card"><h2>Safeguards</h2><p>Every command is tenant/branch scoped, idempotent, version checked, audited and emitted through the durable outbox. Provider calls remain outside database transactions.</p></aside></main>;
}
