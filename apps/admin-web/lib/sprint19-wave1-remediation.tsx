...
function QueueBoard() {
  ...
  return <Shell accessibilityTitle="Operational board" eyebrow="Queue board" title="Điều phối salon theo thời gian thực" description="Realtime chỉ là tín hiệu refetch; PostgreSQL vẫn là nguồn sự thật." actions={<a className="s19-button s19-button-primary" href="/admin/operations/walk-ins/new">+ Tạo walk-in</a>}><div className="s19-toolbar"><Field label="Chi nhánh"><select aria-label="Branch" value={selectedBranch} onChange={(event) => setSelectedBranch(event.target.value)}><option value="">Chọn chi nhánh</option>{rows(branch.data).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field><button className="s19-button s19-button-secondary" onClick={board.load}>Làm mới board</button><span className="s19-live-indicator"><span />Live refetch</span></div><p className="s19-helper">Data version: refetch signal from PostgreSQL</p><StatePanel state={board.state} error={board.error} retry={board.load} label="queue board" />{board.state === "ready" ? <div className="s19-board-grid">{columns.map((column) => <section className="s19-board-column" key={column}><header><div><span className="s19-column-kicker">{column === "WAITING" ? "Hàng đợi" : "Lịch vận hành"}</span><h2>{column.replaceAll("_", " ")}</h2></div><span>{board.data?.columns?.[column]?.length ?? 0}</span></header>{(board.data?.columns?.[column] ?? []).map((item: any) => <article className="s19-board-card" key={item.id}><div className="s19-board-card-top"><strong>{item.bookingReference ?? `#${item.queueNumber ?? "—"}`}</strong><span className="s19-status s19-status-info">{item.status}</span></div><h3>{item.customerDisplayName ?? item.displayName ?? "Khách hàng"}</h3><p>{item.serviceName ?? item.services?.[0]?.name ?? "Dịch vụ"}</p><small>{item.startAt ? new Date(item.startAt).toLocaleTimeString("vi-VN") : item.estimatedWaitMinutes != null ? `ETA ${item.estimatedWaitMinutes} phút` : "Chưa có ETA"}</small><a className="s19-inline-action" href={item.sessionId ? `/admin/service-sessions/${item.sessionId}` : `/admin/appointments/${item.id}/overview`}>Mở chi tiết →</a></article>)}{!(board.data?.columns?.[column] ?? []).length ? <p className="s19-column-empty">Không có bản ghi</p> : null}</section>)}</div> : null}</Shell>;
}
