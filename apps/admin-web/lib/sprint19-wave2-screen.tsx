...
function CreditNotes({ detailId }: { detailId?: string }) { const remote = useRemote(detailId ? `/v1/credit-notes/${detailId}` : "/v1/credit-notes"); const intents = useIntentKeys(); if (detailId) return <Page title="Credit note detail" eyebrow="CORRECTIONS · CREDIT NOTE" description="Credit notes are immutable evidence. Delivery and application remain separate server actions." actions={<a className="w2-button w2-button-secondary" href="/admin/credit-notes">Back to credit notes</a>}><AsyncPanel value={remote} label="credit note" />{remote.state === "ready" && <div className="w2-card"><div className="w2-card-head"><div><p className="w2-eyebrow">CREDIT NOTE</p><h2>{remote.data.creditNoteNumber ?? remote.data.id}</h2></div><Status value={remote.data.status} /></div><dl className="w2-totals"><div><dt>Invoice</dt><dd>{remote.data.invoiceNumber ?? remote.data.invoiceId}</dd></div><div><dt>Total</dt><dd>{money(remote.data.totalMinor ?? remote.data.amountMinor, remote.data.currency)}</dd></div></dl><pre className="w2-evidence">{JSON.stringify(remote.data, null, 2)}</pre><button className="w2-button w2-button-secondary" onClick={() => { const intent = "deliver-credit-note"; void command(`/v1/credit-notes/${detailId}/deliver`, { channel: "PRINT" }, intents.key(intent)).then(() => intents.clear(intent)); }}>Print / deliver evidence</button></div>}</Page>;
