...
function NewSale({ appointmentId }: { appointmentId?: string }) {
  ...
  async function createOrder() {
    ...
    try { setOrder(await command(`/v1/appointments/${input.trim()}/pos-orders`, {}, intents.key(intent))); intents.clear(intent); setNotice("Order opened from the immutable appointment service snapshots."); } catch (reason: any) { setError(reason.message); }
  }
...
function OrderWorkspace({ orderId }: { orderId: string }) {
  ...
  async function mutate(path: string, body: any, success: string, intent = path): Promise<any> {
    ...
    try { const result = await command(path, body, intents.key(intent)); intents.clear(intent); setNotice(success); await order.load(); return result; } catch (reason: any) { setError(reason.code === "VERSION_CONFLICT" ? "Version conflict: the order changed elsewhere. It has been refreshed." : reason.message); await order.load(); return undefined; } finally { setSaving(false); }
  }
...
function CashSession({ sessionId, close = false }: { sessionId: string; close?: boolean }) {
  ...
  async function act(path: string, body: any, success: string) { if (!value.data) return; try { await command(path, { version: value.data.version, ...body }, intents.key(path)); intents.clear(path); setMessage(success); await value.load(); await movements.load(); await review.load(); } catch (reason: any) { setError(reason.code === "VERSION_CONFLICT" ? "Version conflict: the session was refreshed." : reason.message); await value.load(); } }
