...
export default function LoginPage() {
  ...
  return <main className="ns-login"><section className="ns-login__panel"><p className="ns-eyebrow">NAILSOFT ADMIN</p><h1>Đăng nhập</h1><p>Quản lý vận hành salon theo tenant và chi nhánh.</p>{state !== "workspace" ? <form onSubmit={submit} aria-busy={state === "loading"}><label>Email<input name="email" type="email" autoComplete="email" required /></label><label>Mật khẩu<input name="password" type="password" autoComplete="current-password" required /></label><button disabled={state === "loading"}>{state === "loading" ? "Đang đăng nhập..." : "Đăng nhập"}</button></form> : <section className="ns-login__workspaces"><h2>Chọn workspace</h2>{workspaces.length === 0 ? <p>Không có workspace đang hoạt động.</p> : <ul>{workspaces.map((workspace) => <li key={workspace.membershipId}><button onClick={() => void choose(workspace.membershipId)}>{workspace.name}</button><small>{workspace.slug}</small></li>)}</ul>}</section>}{state === "mfa" ? <p role="status">{message} <a href="/auth/mfa">Tiếp tục xác minh</a></p> : null}{state === "error" ? <div role="alert"><p>{message}</p><button onClick={() => setState("idle")}>Thử lại</button></div> : null}{state === "success" ? <p role="status">{message}</p> : null}</section></main>;
}
