...
export default function LoginPage() {
  ...
  async function choose(membershipId: string) {
    setState("loading");
    try { const data = await selectWorkspace(workspaceToken, membershipId); if (data.authenticationState) { setState("mfa"); setMessage(data.authenticationState === "MFA_ENROLLMENT_REQUIRED" ? "Activate MFA before continuing." : "Enter your MFA code to continue."); return; } setState("success"); setMessage("Workspace selected."); router.replace("/admin/dashboard"); }
    catch (error) { setState("error"); setMessage(error instanceof Error ? error.message : "Unable to select the workspace."); }
  }
  return <main className="ns-login"><section className="ns-login__panel"><p className="ns-eyebrow">NAILSOFT ADMIN</p><h1>{copy.title}</h1><p>{copy.description}</p>{state !== "workspace" ? <form onSubmit={submit} aria-busy={state === "loading"}><label>Email<input name="email" type="email" autoComplete="email" required /></label><label>{copy.password}<input name="password" type="password" autoComplete="current-password" required /></label><button disabled={state === "loading"}>{state === "loading" ? copy.signingIn : copy.signIn}</button></form> : <section className="ns-login__workspaces"><h2>{copy.selectWorkspace}</h2>{workspaces.length === 0 ? <p>{copy.noWorkspace}</p> : <ul>{workspaces.map((workspace) => <li key={workspace.membershipId}><button onClick={() => void choose(workspace.membershipId)}>{workspace.name}</button><small>{workspace.slug}</small></li>)}</ul>}</section>}{state === "mfa" ? <p role="status">{message} <button type="button" className="ns-login__mfa-link" onClick={() => router.push("/auth/mfa")}>{copy.continueVerification}</button></p> : null}{state === "error" ? <div role="alert"><p>{message}</p><button onClick={() => setState("idle")}>{copy.tryAgain}</button></div> : null}{state === "success" ? <p role="status">{message}</p> : null}</section></main>;
}
