...
export class AccountingController {
  ...
  @Post("source-postings") @RequirePermission("accounting.source_posting.create") sourcePosting(@Body() b:any,@Headers("idempotency-key") k:string,@Req() r:AuthenticatedRequest){return this.s.createSourcePosting(r.auth,b,rid(r),idem(k)).then(x=>ok(x,r));}
  @Get("source-mappings") @RequirePermission("accounting.source_mapping.read") sourceMappings(@Query("bookId") bookId:string,@Req() r:AuthenticatedRequest){return this.s.sourceMappings(r.auth,bookId).then(x=>ok(x,r));}
  @Post("source-mappings") @RequirePermission("accounting.source_mapping.manage") createSourceMapping(@Body() b:any,@Req() r:AuthenticatedRequest){return this.s.createSourceMapping(r.auth,b,rid(r)).then(x=>ok(x,r));}
  ...
  @Post("bank-matches/:id/unmatch") @RequirePermission("accounting.bank_match.manage") unmatchBankMatch(@Param("id") id:string,@Body() b:any,@Req() r:AuthenticatedRequest){return this.s.transitionBankMatch(r.auth,id,"VOIDED",b,rid(r)).then(x=>ok(x,r));}
  @Post("bank-reconciliations") @RequirePermission("accounting.bank_reconciliation.manage") createBankReconciliation(@Body() b:any,@Req() r:AuthenticatedRequest){return this.s.createBankReconciliation(r.auth,b,rid(r)).then(x=>ok(x,r));}
  @Post("bank-reconciliations/:id/start-matching") @RequirePermission("accounting.bank_reconciliation.manage") startMatching(@Param("id") id:string,@Body() b:any,@Req() r:AuthenticatedRequest){return this.s.transitionBankReconciliation(r.auth,id,"MATCHING",b,rid(r)).then(x=>ok(x,r));}
  @Post("bank-reconciliations/:id/submit-review") @RequirePermission("accounting.bank_reconciliation.manage") submitReview(@Param("id") id:string,@Body() b:any,@Req() r:AuthenticatedRequest){return this.s.transitionBankReconciliation(r.auth,id,"REVIEW",b,rid(r)).then(x=>ok(x,r));}
  @Post("bank-reconciliations/:id/reconcile") @RequirePermission("accounting.bank_reconciliation.manage") reconcile(@Param("id") id:string,@Body() b:any,@Req() r:AuthenticatedRequest){return this.s.transitionBankReconciliation(r.auth,id,"RECONCILED",b,rid(r)).then(x=>ok(x,r));}
  ...
  @Post("statement-snapshots/:id/approve") @RequirePermission("accounting.statement.approve") approveSnapshot(@Param("id") id:string,@Body() b:any,@Req() r:AuthenticatedRequest){return this.s.transitionStatementSnapshot(r.auth,id,"APPROVED",b,rid(r)).then(x=>ok(x,r));}
  @Post("statement-snapshots/:id/finalize") @RequirePermission("accounting.statement.approve") finalizeSnapshot(@Param("id") id:string,@Body() b:any,@Req() r:AuthenticatedRequest){return this.s.transitionStatementSnapshot(r.auth,id,"FINAL",b,rid(r)).then(x=>ok(x,r));}
}
