...
(async () => {
  ...
  for (const [name, viewport] of viewports) {
    ...
    await page.goto(`${adminBaseUrl}/admin/staff/${staffId}/pay-profile`, { waitUntil: "networkidle" });
    await page.getByRole("heading", { name: "Hồ sơ lương nhân sự" }).waitFor({ timeout: 15_000 });
    ...
    await page.screenshot({ path: `artifacts/ui-completion/admin-staff-pay-profile/${name}.png`, fullPage: true });
  }
  await browser.close();
  console.log(JSON.stringify({ route: `/admin/staff/${staffId}/pay-profile`, viewports: results }));
