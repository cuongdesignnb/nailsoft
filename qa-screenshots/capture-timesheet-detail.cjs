...
const timesheetId = process.env.TIMESHEET_ID || "f1200000-0000-4000-8000-000000000060";
...
(async () => {
  ...
  for (const [name, viewport] of viewports) {
    ...
    await page.goto(`${adminBaseUrl}/admin/timesheets/${timesheetId}`, { waitUntil: "networkidle" });
    await page.getByRole("heading", { name: "Chi tiết bảng công" }).waitFor({ timeout: 15_000 });
    ...
    await page.screenshot({ path: `artifacts/ui-completion/admin-timesheet-detail/${name}.png`, fullPage: true });
  }
  await browser.close();
  console.log(JSON.stringify({ route: `/admin/timesheets/${timesheetId}`, viewports: results }));
