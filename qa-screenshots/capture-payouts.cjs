...
(async () => {
  ...
  for (const [name, viewport] of viewports) {
    ...
    await page.goto(`${adminBaseUrl}/admin/payouts`, { waitUntil: "networkidle" });
    await page.getByRole("heading", { name: "Đợt chi trả", exact: true }).waitFor({ timeout: 15_000 });
    ...
    await page.screenshot({ path: `artifacts/ui-completion/admin-payouts/${name}.png`, fullPage: true });
  }
  await browser.close();
  console.log(JSON.stringify({ route: "/admin/payouts", viewports: results }));
