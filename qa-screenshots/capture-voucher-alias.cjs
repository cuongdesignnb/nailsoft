...
(async () => {
  ...
  for (const [name, viewport] of viewports) {
    ...
    await page.getByRole("heading", { name: "Voucher khách hàng", exact: true }).waitFor({ timeout: 15_000 });
    ...
    await page.screenshot({ path: `artifacts/ui-completion/admin-vouchers/${name}.png`, fullPage: true });
  }
  await browser.close();
  console.log(JSON.stringify({ route, viewports: results }));
