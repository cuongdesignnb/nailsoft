...
(async () => {
  ...
  for (const [name, viewport] of viewports) {
    ...
    await page.getByRole("heading", { name: "Tổng quan vận hành", exact: true }).waitFor({ timeout: 15_000 });
    ...
    await page.screenshot({ path: `artifacts/ui-completion/admin-dashboard/${name}.png`, fullPage: true });
  }
  await browser.close();
