...
(async () => {
  ...
  for (const [name, viewport] of viewports) {
    ...
    await page.goto(`${adminBaseUrl}/admin/pos`, { waitUntil: "networkidle" });
    await page.getByRole("heading", { name: /POS|thu ngân/i }).first().waitFor({ timeout: 15_000 });
    ...
    await page.screenshot({ path: `artifacts/ui-completion/admin-pos/${name}.png`, fullPage: true });
  }
  await browser.close();
  console.log(JSON.stringify({ route: "/admin/pos", viewports: results }));
