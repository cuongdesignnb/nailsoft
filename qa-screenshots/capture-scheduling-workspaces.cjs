...
const routes = [
  { path: "/admin/scheduling/shifts", heading: "Lập lịch ca", dir: "admin-scheduling-shifts" },
  { path: "/admin/scheduling/leave-requests", heading: "Duyệt đơn nghỉ", dir: "admin-scheduling-leave-requests" },
];
...
(async () => {
  ...
  for (const route of routes) {
    for (const [name, viewport] of viewports) {
      ...
      await page.getByRole("heading", { name: route.heading }).waitFor({ timeout: 15_000 });
      ...
      results.push({
        route: route.path,
        name,
        loadingVisible: bodyText.includes("Đang tải không gian làm việc"),
        englishHeadingVisible: bodyText.includes("Shift planner") || bodyText.includes("Leave review"),
  ...
  if (results.some((item) => item.loadingVisible || item.englishHeadingVisible || item.violations.length || item.layout.documentWidth !== item.layout.viewportWidth || item.layout.bodyWidth !== item.layout.viewportWidth)) process.exitCode = 1;
})().catch((error) => {
