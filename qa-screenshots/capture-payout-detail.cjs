...
const payoutId = process.env.PAYOUT_ID || "f1200000-0000-4000-8000-000000000095";
...
(async () => {
  ...
  for (const [name, viewport] of viewports) {
    ...
    await page.goto(`${adminBaseUrl}/admin/payouts/${payoutId}`, { waitUntil: "networkidle" });
    await page.getByRole("heading", { name: "Chi tiết đợt chi trả" }).waitFor({ timeout: 15_000 });
    ...
    await page.screenshot({ path: `artifacts/ui-completion/admin-payouts-detail/${name}.png`, fullPage: true });
  }
  await browser.close();
  console.log(JSON.stringify({ route: `/admin/payouts/${payoutId}`, viewports: results }));
