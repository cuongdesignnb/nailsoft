...
describe("Sprint 16 PostgreSQL asset invariants", () => {
  ...
  it("keeps module opt-in and source generation unique", async () => {
    ...
    const category = (await db.query<any>("INSERT INTO asset_categories(tenant_id,code,name) VALUES($1,'QA16','QA Assets') RETURNING id", [tenant])).rows[0];
    const candidate = (await db.query<any>("INSERT INTO asset_candidates(tenant_id,branch_id,candidate_number,name,currency,eligible_amount_minor) VALUES($1,$2,'QA16-C','QA Asset','VND',1000) RETURNING id", [tenant, branch])).rows[0];
  ...
  it("rejects economics mutation after activation and protects count snapshots", async () => {
    const category=(await db.query<any>("SELECT id FROM asset_categories WHERE tenant_id=$1 AND code='QA16' LIMIT 1",[tenant])).rows[0];
    const asset=(await db.query<any>("INSERT INTO assets(tenant_id,branch_id,category_id,asset_code,name,status,currency,gross_carrying_amount_minor,residual_value_minor) VALUES($1,$2,$3,'QA16-A','QA Asset','ACTIVE','VND',1000,100) RETURNING id",[tenant,branch,category.id])).rows[0];
