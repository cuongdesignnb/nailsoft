...
describe("Sprint 18 production readiness controls", () => {
  it("keeps the migration history immutable and supplies operational scripts", () => {
    ...
    for (const file of ["scripts/backup.mjs", "scripts/restore.mjs", "scripts/integrity-check.mjs", "scripts/generate-release-manifest.mjs", "scripts/generate-sbom.mjs", "scripts/security-scan.mjs"]) expect(existsSync(file)).toBe(true);
  });
